...
{
       0,    54,    54,    58,    59,    62,    67,    71,    75,    79,
      80,    84,    91,    92,    96,   107,   115,   116,   121,   128,
     136,   144,   153,   161,   166
};
#endif
...
static const char *const yytname[] =
{
  ...
  "GREATAMP", "GREATGREAT", "GREATGREATAMP", "LESS", "PIPE", "AMPERSAND",
  "$accept", "goal", "commands", "command", "simple_command", "pipe_list",
  "command_and_args", "argument_list", "argument", "command_word",
  "iomodifier_list", "iomodifier", "background_optional", YY_NULLPTR
};
#endif
...
#line 96 "shell.y" /* yacc.c:1646  */
    {
		...
		if (!(strchr((yyvsp[0].string_val), '*') == NULL && strchr((yyvsp[0].string_val), '?') == NULL)) {
			 wildExpansionNecessary((yyvsp[0].string_val));
		}
		else 
			tildeOrEnv((yyvsp[0].string_val));
		}
