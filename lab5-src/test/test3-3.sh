...
if [[ $proccount != "1" ]] || [[ $threadcount != "5" && $threadcount != "6" ]]; then
    echo "Expected 1 process and 5 (or 6) threads after 3 requests; got $proccount processes and $threadcount threads" > $cmpfile
    ret=1
fi
...
if [[ $proccount != "1" ]] || [[ $threadcount != "5" && $threadcount != "6" ]]; then
    echo "Expected 1 process and 5 (or 6) threads after 8 requests; got $proccount processes and $threadcount threads" > $cmpfile
    ret=1
fi

for pid in $nc1 $nc2 $nc3 $nc4 $nc5 $nc6 $nc7 $nc8 $server_pid; do
